...
const FormMockInterview = ({ initialData }: FormMockInterviewProps) => {
  ...
  const cleanAiResponse = (responseText: string) => {
    // 1. trim any surrounding space
    let cleanText = responseText.trim();

    // 2. remove occurences of "json" or code block symbols
    cleanText = cleanText.replace(/(json|```|`)/g, "");

    // 3. extract json array by capturing text between square brackets
  ...
  const generateAIResponse = async (data: FormData) => {
    ...
    const aiResult = await generateContentStream(prompt);
    // console.log(aiResult.trim())
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      if (initialData) {
        ...
        if (isValid) {
          const aiResult = await generateAIResponse(data)
          // console.log(aiResult, "this is from if")
          await addDoc(collection(db, "interviews"), { ...data, userId, questions: aiResult, createdAt: serverTimestamp() })
          toast(toastMessage.title, { description: toastMessage.description })
        }
      }
  ...
  return (
    <div className="w-full flex-col space-y-4">
      ...
      <div className="mt-4 flex items-center justify-between w-full">
        ...
        {!initialData && (
          <Button size={"icon"} variant={"ghost"} style={{ backgroundColor: "transparent", outline: "none" }}             onMouseEnter={e => e.currentTarget.style.borderColor = "gray"}
