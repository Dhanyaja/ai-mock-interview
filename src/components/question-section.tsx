      ...
      >
        <TabsList className='bg-transparent w-full flex flex-wrap items-center justify-start gap-4'>
          {questions.map((tab, i) => (
            <TabsTrigger
              ...
              key={tab.question}
              value={tab.question}
              style={{ backgroundColor: "white", outline: "none", border: "none" }}
              onSelect={e => e.currentTarget.style.backgroundColor = "emerald-200"}
        ...
        {questions?.map((tab, i) => (
          <TabsContent key={i} value={tab.question}>
            <p className='text-base text-left tracking-wide textneutral-500'>
              {tab.question}
            </p>
